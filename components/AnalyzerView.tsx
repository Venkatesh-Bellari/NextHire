...
const AnalyzerView = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!isOnline) {
      setError('Analysis requires an internet connection.');
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      const analysisResult = await analyzeResume(resumeText, jobDescriptionText);
  ...
  }, [resumeText, jobDescriptionText, isOnline]);

  const isDisabled = isLoading || isFileProcessing;
  const isButtonDisabled = isDisabled || !isOnline || !resumeText || !jobDescriptionText;
    ...
    <div className="max-w-5xl mx-auto">
      <div className="text-left mb-10">
        ...
        <p className="mt-2 text-lg text-slate-300">
          Analyze your resume against a job description for an instant alignment score and feedback.
        </p>
      </div>
      ...
      <div className="bg-slate-800/50 backdrop-blur-xl p-6 sm:p-8 rounded-2xl shadow-xl border border-white/10">
        <div className="space-y-8">
          <div>
            <label className="mb-2 font-semibold text-slate-200 block">1. Your Resume</label>
             <input
                type="file"
                ref={fileInputRef}
          ...
          <div className="flex flex-col">
            <label htmlFor="job-description" className="mb-2 font-semibold text-slate-200">2. Job Description</label>
            <textarea
              id="job-description"
              value={jobDescriptionText}
              onChange={(e) => setJobDescriptionText(e.target.value)}
              placeholder="Paste the full text of the job description here..."
              className="w-full h-64 p-4 border border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 resize-y text-white bg-slate-700/50 placeholder:text-slate-400"
              disabled={isDisabled}
            />
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          >
            {isLoading ? <Spinner /> : 'Analyze My Chances'}
          </button>
          {!isOnline && (
            <p className="mt-2 text-sm text-amber-400">Analysis requires an internet connection.</p>
          )}
        </div>
      </div>
